...
@app.put("/ReturningAddItem/{IID}/{Quantity}", response_model=dict)
async def Returning_Item_And_Adding_Quantity(
    IID: str,
    Quantity:str,
    db=Depends(get_db)
):
    query = "UPDATE item SET Quantity = (SELECT Quantity FROM item where ItemID = %s) + %s WHERE ItemID = %s"
    db[0].execute(query, (IID, Quantity, IID))
...
@app.put("/ReturningMinusItem/{IID}/{Quantity}", response_model=dict)
async def Returning_Item_And_Adding_Quantity(
    IID: str,
    Quantity:str,
    db=Depends(get_db)
...
@app.post("/CreateBorrowTransaction/{ID}/{IID}/{Quantity}")
async def Create_Borrow_Transaction(
    ...
    db[0].execute(query_get_quantity, (IID,))
    ...
    db[0].execute(query_update_quantity, (new_quantity, IID))
    db[1].commit()
    ...
    db[0].execute(query_insert_transaction, (ID, IID, BorrowedDateTime, Quantity, Deadline, Status))
    db[1].commit()
...
async def Update_Item(
    ...
    cursor, db_connection = db
    cursor.execute(
        "UPDATE item SET ItemName = %s, FromLab = %s, ItemDescription = %s, Quantity = %s, Total_Quantity = %s WHERE ItemID = %s",
        (item.ItemName, item.FromLab, item.ItemDescription, item.Quantity, item.Quantity,  ItemID)
    )
    db_connection.commit()
    db_connection.close()
    return item

@app.put("/AcceptRequest/{ItemID}")
async def Update_Item(
    ItemID : int,
    db=Depends(get_db)
):
